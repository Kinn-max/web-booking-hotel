...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  if (!empty($destination)) {
    $destination = $mysqli->real_escape_string(mb_convert_encoding($destination, 'UTF-8', mb_detect_encoding($destination)));
    $where .= " AND (h.address LIKE '%$destination%' OR c.name LIKE '%$destination%')";
  }
  if (!empty($bed)) {
...
// Thực hiện truy vấn
$sql_city_final = mysqli_query($mysqli, $sql_city);
    ...
    <div class="group-search">
      <div class="group-filter">
        <div class="group-filter--search" style="display: flex;">
          ...
          <div class="icon-search--filter style-search">
            <a id="search-btn">
                ...
                <div class="group-item">
                  ...
                  <div class="right-item">
                    <div class="right-item--top">
                      <div class="right-item--top--left">
                        <h3 class="title-name--hotel"><?php echo $result['name']   ?></h3>
                        <span class="text-span"><?php echo $nameSearch; ?></span>
                      </div>
                      <div class="right-item--top--right">
                        <div class="feedback">
                          <div>Xuất sắc</div>
                          <span style="font-size: 0.8rem">4 đánh giá</span>
                        </div>
                        <span class="point"> 9.2 </span>
                      </div>
                    </div>
                    <span style="font-size: 0.8rem; color: #1a1a1a">
                      Địa chỉ:
                      <?php echo $result['address']   ?>
                    </span>
                    <?php
                    $sql_room = "SELECT * FROM room WHERE id_hotel = " . $result['id'] . " ORDER BY price ASC LIMIT 1";
                    $sql_room_final = mysqli_query($mysqli, $sql_room);
                    $count = mysqli_num_rows($sql_room_final);
                    if ($count > 0) {
                      $room = mysqli_fetch_array($sql_room_final);
                    ?>
                      <div class="right-item--bot">
                        <div class="right-item--bot--left">
                          <span class="item-name"><?php echo $room["name"]   ?></span>
                        </div>
                        <div
                          class="right-item--bot--left"
                          style="text-align: end; color: #1a1a1a;">
                          <span style="font-size: 0.8rem">1 đêm, <?php echo $room["num_of_people"] ?> người</span>
                          <h3 style="font-size: 1.3rem">VND <?php echo $room["price"] ?></h3>
                          <span style="font-size: 0.9rem">Đã bao gồm thuế và phí</span>
                        </div>
                      </div>

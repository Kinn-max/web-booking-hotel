...
if (isset($_POST['change_password'])) {
    ...
    if ($row_password['password'] != $currentPassword) {
        ...
        if ($result_password) {
            ...
            $_SESSION['error_message'] = "Đã xảy ra lỗi khi cập nhật mật khẩu. Vui lòng thử lại.";
        }
    }
...
$query_booking = "SELECT 
    ...
    image_hotel.name AS image_name
FROM 
    booking
JOIN 
    room ON booking.id_room = room.id
JOIN 
    hotel ON room.id_hotel = hotel.id
JOIN 
    city ON hotel.id_city = city.id
LEFT JOIN 
    image_hotel ON hotel.id = image_hotel.id_hotel
WHERE 
    booking.id_user = ?
AND 
    image_hotel.id = (
        SELECT MIN(id) FROM image_hotel WHERE id_hotel = hotel.id
    )";

$stmt_booking = $mysqli->prepare($query_booking);

if ($stmt_booking) {
    $stmt_booking->bind_param("i", $this_id);
    ...
    echo "Lỗi câu truy vấn: " . $mysqli->error;
}
?>
...
<div class="main">
    <div class="sidebar">
        <ul>
            <li><a href="user.php?id=<?php echo $this_id?>&&page=profile"><span class="icon">👤</span>Thông tin cá nhân</a></li>
            <li><a href="user.php?id=<?php echo $this_id?>&&page=book-history"><span class="icon">💼</span>Lịch sử chuyến đi</a></li>
            <li><a href="user.php?id=<?php echo $this_id?>&&page=change-password"><span class="icon">🔒</span>Đổi mật khẩu</a></li>
    ...
    <div class="main-content">
        <?php
        $page_id = isset($_GET['page']) ? $_GET['page'] : 'profile';
